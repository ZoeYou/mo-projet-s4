...
* Note that the *trip_duration* variable has a minimum of 1 second and a maximum of 3,526,282 seconds (approximately 980 hours). It is impossible for someone to make such a long trip. And a trip of a second goes nowhere.
It is therefore clear that there are some outliers associated with the `trip_duration` variable. We will exclude data that lies outside two standard deviations from the mean for example.                   

```{r}
m <-  mean(train$trip_duration)
s <-  sd(train$trip_duration)
train <-  filter(train, trip_duration <= m + 2*s)
train <-  filter(train, trip_duration >= 60)
summary(train$trip_duration)
```
...
direction <- function(lat1, lat2, lng1, lng2)
{
  r <- 6371 # Average Earth Radius
  lat1 <- deg2rad(lat1)
  lat2 <- deg2rad(lat2)
...
We will use the previous functions to calculate the distances(in kilometers) and the directions(in degres), which we will store in the train and test tables.
...
train$direction <- direction(train$pickup_latitude, train$dropoff_latitude, train$pickup_longitude, train$dropoff_longitude)

test$direction <- direction(test$pickup_latitude, test$dropoff_latitude, test$pickup_longitude, test$dropoff_longitude)
```
...
After looking at the distribution of log (distance), we found that its head and middle are well in line with the trend of normal distribution, but there are several peaks in the data set at the tail, which requires us to do some further exploration. 
...
```{r tp_decide_k, message=FALSE, warning=FALSE}
# k.max <- 15
# data <- train_coord_pickup
# wss <- sapply(1:k.max, 
...
Also, it appears that the number of rides with six passengers are higher than the ones with four passengers. That may be due to the fact that those passengers maybe share the cost of the ride, or that there are not enough taxi cabs.   
...
```{r variable_weather}
weather <- read.csv(file = "weather_data_nyc_centralpark_2016(1).csv")
summary(weather)
glimpse(weather)
```
...
weather <- weather %>%
  mutate(date = dmy(date),
         ...
         snow.fall = as.numeric(ifelse(snow.fall == "T", "0.01", snow.fall)),
         snow.depth = as.numeric(ifelse(snow.depth == "T", "0.01", snow.depth)),
         all_precip = snow.fall + precipitation,
         has_snow = (snow.fall > 0.01) | (snow.depth > 0.01),
         has_rain = precipitation > 0.01)
```

Now we can join this to our train data :   

```{r}
plus <- weather[,which(colnames(weather)%in%c("date", "precipitation", "snow.fall", "snow.depth", "all_precip", "has_snow", "has_rain", "maximum.temperature", "minimum.temperature", "average.temperature"))]
...
testDat <- test[,-which(colnames(test)%in%c("id", "pickup_datetime", "dropoff_datetime", "pickup_longitude", "pickup_latitude", "dropoff_longitude", "dropoff_latitude", "distance", "direction", "dateWeather", "maximum.temperature", "minimum.temperature", "average.temperature", "all_precip"))]
testDat$distance <- test$distance
testDat <- testDat[,c(1, 2, 3, 6, 7, 8, 4, 5, 9, 10, 11, 12, 13, 14)] # Garder le même ordre de colonnes que dans dat
```
...
testDatReg <- testDat
testDatReg$has_rain <- as.numeric(as.factor(testDatReg$has_rain))
testDatReg$has_snow <- as.numeric(as.factor(testDatReg$has_snow))

testDatReg <- cbind(rep(1, nrow(testDat)), testDat)
Y_testreg = data.matrix(testDatReg)%*%regmod$coefficients
